...
class openni_camera():
    def __init__(self,display=True):
        ...
        rospy.Subscriber("/videofile/image_raw", Img, self.camera_callback)

    def camera_callback(self,data):
        cv_image = numpify(data)
        self.cv_image = cv_image
        if self.display:
            cv2.imshow('videofile',self.cv_image)
            cv2.waitKey(1)
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            yolo_chair,bbox_rescaled = utils.draw_boxes(frame, boxes_backpack, scores_backpack, labels_backpack, classes, (IMAGE_H, IMAGE_W), show=False)
            ...
            for i in range(len(bbox_rescaled)):
                ...
                # with either hands
                if(abs(bag_finder.l_shoulder.x-bag_finder.l_wrist.x)<50 and abs(bag_finder.r_shoulder.x-bag_finder.r_wrist.x)<20):
